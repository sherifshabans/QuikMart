...
@Composable
fun ProductScreen(viewModel: ProductViewModel = viewModel(), paddingValues: PaddingValues) {
            ...
            topBar = {
                ...
                if (selectedProduct == null) {
                    ProductList(
                        ...
                        onProductClicked = { selectedProduct = it }
                    )
                } else {
                    ProductDetails(selectedProduct!!) {
                        selectedProduct = null // Reset selectedProduct after viewing details
                    }
                }
            })
...
fun ProductList(
    ...
    LazyVerticalGrid(columns = GridCells.Fixed(2), modifier = modifier) {
        items(products) {it
            ProductCard(it, onAddProductToCart){  onProductClicked(it) } }
    }
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ProductCard(product: Product, onAddProductToCart: (Product) -> Unit,    onProductClicked: () -> Unit) {
    ...
    Card(
        ...
        onClick =  onProductClicked // Handle click event here
