...
fun CartScreen(
    ...
    val total by cartViewModel.total.observeAsState(0.0)
    ...
    Column(
        ...
        if (cartItems.isEmpty()) {
            ...
            CartItemsList(
                onDecreaseQuantity = { item -> cartViewModel.updateItem(item, -1) },
                onIncreaseQuantity = { item -> cartViewModel.updateItem(item, 1) },
                onRemoveCartItem = { item -> cartViewModel.updateItem(item, -item.quantity) },
                cartItems = cartItems,
                total = total,
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun CartScreen2(paddingValues: PaddingValues, onCheckOut: () -> Unit) {
