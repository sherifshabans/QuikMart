...
object CartRepository {
    ...
    fun addCartItems(cartItemsToAdd: List<CartItem>) {
        val updatedCart = mutableListOf<CartItem>().apply {
            addAll(cartItems)
            for (item in cartItemsToAdd) {
                val existingItem = find { it.productId == item.productId }
